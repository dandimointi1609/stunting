            ...
            layer.on('click', (e)=>{
                ...
                $.getJSON('titik/lokasi/'+feature.properties.kd_kecamatan, function(detail){
                    $.each(detail, function(index){
                        ...
                        var html='<h6> <button type="button"   class="btn btn-primary" data-toggle="modal" data-target="#basicModal">Lihat Detail</button> <h6>';
                                

                                var data = {
                                labels: [detail[index].nama_desa],
                                // labels: [@foreach ($lokasi as $data)
                                // '{{$data->kd_kecamatan}}',@endforeach],
